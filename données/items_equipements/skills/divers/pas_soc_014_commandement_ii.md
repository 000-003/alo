# Commandement II

## Identification Cardinal
- **Skill_ID** : `PAS_SOC_014`
- **Catégorie** : Passive — Social
- **Type** : Passive (équipée passivement)

## Rangs
| Rang | Bonus | Condition d'acquisition |
|---|---|---|
| I | +2% | Niv. 5 + 500 Yrds |
| II | +5% | Niv. 20 + 2 000 Yrds + Maîtrise préalable |
| III | +8% | Niv. 40 + 8 000 Yrds + Quête de maîtrise |

## Acquisition
- **Enseignant** : Gorim, Commandant de Granzam `NPC_GRA_02`
- **Prix d'apprentissage** : 1 000 Yrds
- **Commande** : `!learn_skill PAS_SOC_014`
- Limite d'équipement : 2 passives du même domaine max

## Lore (Encyclopédie d'Argo)
Diriger, ce n'est pas crier plus fort ; c'est parler juste assez pour être entendu de tous.

## Commande IA
- `SYS_GRANT_PASSIVE(Avatar_ID, 'PAS_SOC_014')`
