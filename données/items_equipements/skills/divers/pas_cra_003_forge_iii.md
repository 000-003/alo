# Forge III

## Identification Cardinal
- **Skill_ID** : `PAS_CRA_003`
- **Catégorie** : Passive — Craft
- **Type** : Passive (équipée passivement)

## Rangs
| Rang | Bonus | Condition d'acquisition |
|---|---|---|
| I | +2% | Niv. 5 + 500 Yrds |
| II | +5% | Niv. 20 + 2 000 Yrds + Maîtrise préalable |
| III | +8% | Niv. 40 + 8 000 Yrds + Quête de maîtrise |

## Acquisition
- **Enseignant** : Lunette, Tailleuse de Freelia `NPC_FRE_05`
- **Prix d'apprentissage** : 4 000 Yrds
- **Commande** : `!learn_skill PAS_CRA_003`
- Limite d'équipement : 2 passives du même domaine max

## Lore (Encyclopédie d'Argo)
La véritable maîtrise de la forge est de savoir quand s'arrêter — le geste de trop peut tout gâcher.

## Commande IA
- `SYS_GRANT_PASSIVE(Avatar_ID, 'PAS_CRA_003')`
