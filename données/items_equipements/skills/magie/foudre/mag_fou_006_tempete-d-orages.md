# Tempête d'Orages

## Identification Cardinal
- **Skill_ID** : `MAG_FOU_006`
- **Catégorie** : Magie — Foudre
- **Tier** : T3 · **Rareté** : Rare
- **Race Affinité** : Imp (+30% efficacité si cette race)

## Paramètres de Combat
| Paramètre | Valeur |
|---|---|
| Coût MP | 300 |
| Temps d'Incantation | 3.0s |
| Cooldown | 60 |
| Niveau Requis | 30 |
| Interruptible (Casting Break) | Oui (si dégâts > 5% Max HP reçus) |

## Effet
Tempête d'orages : éclairs multiples frappant aléatoirement une zone.
**Zone d'effet** : Oui

## Incantation
*Le joueur tape `!cast tempete-d-orages` pour lancer le sort.*

## Acquisition (Enseignement)
- **Enseignant** : Vork NPC_VOU_29
- **Prix d'apprentissage** : 5000 Yrds
- **Commande** : `!learn_skill MAG_FOU_006`

## Lore (Encyclopédie d'Argo)
Tempête maîtrisée par les ingénieurs impishoons.

## Commande IA
- `SYS_GRANT_SPELL(Avatar_ID, 'MAG_FOU_006')`
