# Typhon Dévastateur

## Identification Cardinal
- **Skill_ID** : `MAG_VEN_006`
- **Catégorie** : Magie — Vent
- **Tier** : T3 · **Rareté** : Rare
- **Race Affinité** : Sylph (+30% efficacité si cette race)

## Paramètres de Combat
| Paramètre | Valeur |
|---|---|
| Coût MP | 300 |
| Temps d'Incantation | 3.0s |
| Cooldown | 60 |
| Niveau Requis | 30 |
| Interruptible (Casting Break) | Oui (si dégâts > 5% Max HP reçus) |

## Effet
Typhon dévastateur déchaînant des vents violents sur une large zone.
**Zone d'effet** : Oui

## Incantation
*Le joueur tape `!cast typhon-devastateur` pour lancer le sort.*

## Acquisition (Enseignement)
- **Enseignant** : Sora NPC_SWI_79
- **Prix d'apprentissage** : 5000 Yrds
- **Commande** : `!learn_skill MAG_VEN_006`

## Lore (Encyclopédie d'Argo)
Typhon ayant jadis protégé Swilvane d'une flotte pirate.

## Commande IA
- `SYS_GRANT_SPELL(Avatar_ID, 'MAG_VEN_006')`
