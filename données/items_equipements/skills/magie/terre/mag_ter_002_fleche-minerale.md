# Flèche Minérale

## Identification Cardinal
- **Skill_ID** : `MAG_TER_002`
- **Catégorie** : Magie — Terre
- **Tier** : T1 · **Rareté** : Commune
- **Race Affinité** : Gnome (+30% efficacité si cette race)

## Paramètres de Combat
| Paramètre | Valeur |
|---|---|
| Coût MP | 45 |
| Temps d'Incantation | 1.25s |
| Cooldown | 2 |
| Niveau Requis | 4 |
| Interruptible (Casting Break) | Oui (si dégâts > 5% Max HP reçus) |

## Effet
Flèche minérale : projectile rocheux à fragmentation.

## Incantation
*Le joueur tape `!cast fleche-minerale` pour lancer le sort.*

## Acquisition (Enseignement)
- **Enseignant** : Roan NPC_ALN_69
- **Prix d'apprentissage** : 500 Yrds
- **Commande** : `!learn_skill MAG_TER_002`

## Lore (Encyclopédie d'Argo)
Projectile utilisé par les mineurs gnomes.

## Commande IA
- `SYS_GRANT_SPELL(Avatar_ID, 'MAG_TER_002')`
