...
- **Loot** : nuancé, pas uniforme (précision étape 51). Les **3 boss apex** (`BOSS_YGG_001`, `BOSS_JOT_001`, `BOSS_AIN_100`) sont anti-farm strict : aucun drop monnayable, récompenses = déblocage de zone, titres serveur (`!titre_set`), gravure Monument des Épéistes. Les **4 boss de palier** (`BOSS_AIN_001/027/074/075`) ont une table de loot classique (Yrds de palier, Gemme, Last Attack Bonus lié) — composants liés via `SYS_GRANT_ITEM`.
- **Parties Récoltables (D78, étape 51)** : les 7 fiches portent désormais une section dédiée — items/Last Attack Bonus existants réattachés à un seuil de phase (boss de palier) ou récompense système déjà actée formalisée sans nouvel item (boss apex). Détail : `directives_generation/23_etude_patterns_rarete_drop.md` §1/§4.
- **Commandes** : `!sys_carve_set` (GM) / `SYS_SET_CARVE_TABLE` (IA) ajoutées (D78, propagées pour l'ensemble du roster boss — territoriaux et axe vertical). Le reste est inchangé — `!dungeon_queue`, `!dome_enter`/`!dome_log`, `!sys_spawn_boss`, `SYS_LOG_RAID`, `!bounty`, `!titre_set`, `SYS_GRANT_ITEM` tous existants.
