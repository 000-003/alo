...
**Phase données CLOSE (CDC §10 ✅) — phase P3 OUVERTE (CDC §11, chantier PE `bot/`).** Fronts possibles au choix du PE : (a) **audit de conformité du code `bot/` contre les CDC 16-20** (frontière déterministe L1 seul écrivain, statelessness, interface `generate()`, verrou D22 à l'ingestion RAG) ; (b) backlog documentaire mineur : accessoires (dérogation D39 à arbitrer), auberge exploitable + 2 taverniers + peuplement `T_GUILDS`, `!sys_rag_reindex`/`SYS_RAG_REINDEX` en `[BESOIN_COMMANDE]`, arbitrage `ZONE_ROUTE_LUGRU` ; (c) audit CGU des API gratuites avant lancement public (réserve étape 39).
